...
class SlideController extends Controller
{
    ...
    {
        // abort_unless(\Gate::allows("CREATE SLIDE"), 403);
        if($request->ajax()) {
    ...
    public function copy(Request $request){
        // abort_unless(\Gate::allows("UPDATE SLIDE"), 403);
    ...
    public function setup(Request $request)
    {
        ...
        //save slide setup
        $data->time_show = strip_tags($request->time);
        $data->save();
        if($request->ajax()) {
            return response()->json(['result' => 'Berhasil update data slide']);
        }
    ...
    public function remove(Request $request)
    {
        ...
        return response()->json(['result' => 'Berhasil move slide active to draft']);
    }
    ...
    public function restore(Request $request)
    {
        ...
        Image::onlyTrashed()->whereIn('id', $request->id)->restore();
        return response()->json(['result' => 'Berhasil restore data']);
    }

    public function delete(Request $request)
    {
        abort_unless(\Gate::allows("DELETE SLIDE"), 403);
    ...
    {
        abort_unless(\Gate::allows("DELETE SLIDE"), 403);
        // Storage::disk('s3')->delete($slide->source);
        // $slide->delete();
        // return redirect()->back();
